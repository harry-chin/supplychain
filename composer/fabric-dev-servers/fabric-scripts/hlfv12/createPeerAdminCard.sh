...
PRIVATE_KEY="${DIR}"/composer/crypto-config/peerOrganizations/grower.hc.com/users/Admin@grower.hc.com/msp/keystore/ee3aa9cd2f69f8471c2d7fd00fb503e3a050da7e9833ba1bf884418e844e98b5_sk
CERT="${DIR}"/composer/crypto-config/peerOrganizations/grower.hc.com/users/Admin@grower.hc.com/msp/signcerts/Admin@grower.hc.com-cert.pem
...
"${HL_COMPOSER_CLI}"  card create -p ./fabric-scripts/hlfv12/composer/connection.json -u PeerAdmin -c "${CERT}" -k "${PRIVATE_KEY}" -r PeerAdmin -r ChannelAdmin --file $CARDOUTPUT
